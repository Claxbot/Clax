...
exports.run = (client, msg, args) => {
  ...
  .addField(':warning: Uyarı :warning:', '`kilit-koy` adlı komutu özel mesajlarda kullanamazsın.')
  return msg.author.sendEmbed(ozelmesajuyari); }
  if (!client.lockit) client.lockit = [];
  let time = args.join(' ');
  let validUnlocks = ['release', 'unlock'];
  if (!time) return msg.reply('Doğru kullanım: ' + prefix + 'kilit-koy <süre örneğin: 2 min>');
